...
Plain `ftp` causes an `ECONNRESET` on the data socket when the GitHub Actions runner connects via IPv6 (EPSV mode). The KAS server at `w01*****.kasserver.com` supports `AUTH TLS`, so always use:
